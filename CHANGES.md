...
- **SecurityClassifier.java** (MODIFICADO): Melhorada drasticamente a precisão da classificação. Introduzida a persona de "Auditor Estrito", ajuste nos pesos de similaridade (60% semântica / 40% heurística) e instruções explícitas para tratar comandos de destruição de infraestrutura como `SUSPECT` ou `RISKY`.
